...
async def test_order_pending_to_filled(open_listing, place_order, make_user):
...
def test_order_cannot_transition_from_terminal_state():
    """Once FILLED, there is no reachable mutation route for an order at all:
    no PATCH/PUT/DELETE is registered under /market for orders. Pure route
    introspection -- no DB/client needed, so this stays a sync test."""
...
async def test_duplicate_idempotency_key_returns_same_order_without_double_charging(open_listing, place_order,
                                                                                     make_user):
